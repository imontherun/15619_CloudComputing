...
""" This is a Python3 program. It provides one function called 
...
""" There is a encoding problem with Python. No matter I used the default 
    file encoding (locale.getpreferredencoding(), utf_8 in Linux) or specify
    the encoding in open() (latin1, ascii, utf_8, utf_8_sig) when opening 
    input file, there would be UnicodeDecodeErrors when reading lines from 
    input file (for line in inputFile)
    
    Python3: Using the argument errors = "surrogateescape" in open() can 
    preserve the incorrect bytes and writing them back to output file. 
    Only encoding = ascii could work, other encoding would get incorrect 
    split when split the line (i.e. more or less than 4 columns)
    
    Python2: Can not use the arguments "encoding" and "errors". Could run 
    without exception but get the same filter result as Python3 (after setting
    like above)
    
    Both results get the correct answer of checkpoint, but some characters are
...
def dataFilter(inputPath, outputPath):
    ...
    try:
        ...
        with open(inputPath, "rt", encoding = "ascii",
                  errors = "surrogateescape") as inputFile:
        ...
        with open(outputPath, "wt", encoding = "utf-8",
                  errors = "surrogateescape") as outputFile:
